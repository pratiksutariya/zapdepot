...
{
    // \Log::info("helpers GoogleSheet......inside");
    // \Log::info($data);
    try {
...
function ActiveCampign($user_data){
    try {  
        // \Log::info("copme in helper active campign.......");
...
// print_r(json_decode(json_encode($response->object()),true));
                // dd($response->object());
            }
        } catch (\Throwable$e) {
        \Log::info($e->getMessage());
